...
label    = "O13CS_night469k_"  # Label before index # if JUG data, idx is not used and single file is assumed.
...
startidx =  40     # Starting index, you can ommit or add the leading zeros, it works either way. the file indices must consist of 5 digits.
endidx   = 40      # End index, again, the leading zeros are not nessecary but can be added for fun
...
rebinsize = 0.250
stickspectrum = False #prdouce a stickspectrum with stickthreshold
stick_dopplerread = False  # if this is turned on an approach is made to find doppler pairs based on stick_exampledoppler
##### END OF INPUT


######## Advanced control parameters
rezero = 1.0           # substract noisefloor by the inverse squared value weighted average of each section.
stickthreshold = 2.5   # threshold in terms of global spectrum. local threshold hardwired to 2.5 for three datapoints at and around maximum.
...
stick_exampledoppler = [[ 9736.70304,            0.05451],[18945.27468,        0.10987]] #  - uses two dopplersplittings (+- from centre frequency)  with two frequencies, to calculate the expected dopplersplit throughout the spectrum
