...
import OpenBinary_module_v04062026 as obm
# Created by Sven Herbers 10.07.2024 - Sven_herbers@web.de - Newest update 04.06.2026

##### INPUT

label    = "4MPY_"      # Label before index
startidx =      98280   # Starting index (5-digits at end of file names)
endidx   =      98284   # Ending index. If single filed set endidx to startidx

masklines=      []      # Lines to remove
maskwidth=      0.1     # +-width (MHz) to mask

forcecalc=      True    # False = use existing spectrum, True = force recalculation. For single file recalculation is always forced.
rangeMHz =      1.0     # +-range (MHz) around probe freq read from binary file; For Resonator: ~HWHM. For chirps: ~chirp_width/2
...
def three_point_interpol(tpix,tpiy):
        "simple three point interpolation, for linear or quadratic case. Returns interpolated values 'weightedx, weightedy'"
        resort=np.argsort(tpiy)
        tpi1=tpiy[resort]
        tpi0=tpix[resort]
        ...
        if tpi1[0]/tpi1[2] < 0.5:
            ...
            #weightedy=((tpi1[1]-tpi1[0])**2+(tpi1[2]-tpi1[0])**2)/((tpi1[1]-tpi1[0])+(tpi1[2]-tpi1[0]))+tpi1[0] # old interpol
            tpi0_rel = tpi0 - weightedx
            weightedy = tpi1[2] - (tpi1[0]-tpi1[2])/(tpi0_rel[0]-tpi0_rel[2])*tpi0_rel[2] # the strongest and the weakest point of the three are expected to be on one line
        else:
            ...
            #weightedy=((tpi1[1]-tpi1[0])**3+(tpi1[2]-tpi1[0])**3)/((tpi1[1]-tpi1[0])**2+(tpi1[2]-tpi1[0])**2)+tpi1[0] # old interpol
            tpi0_rel=tpi0-weightedx
            weightedy = tpi1[2]-((tpi1[0]-tpi1[2])/(tpi0_rel[0]**2-tpi0_rel[2]**2))*tpi0_rel[2]**2
...
def line_at_cursor(event):
    if event.dblclick == True:
        ...
        valuex, valuey = three_point_interpol(x[idxmax-1:idxmax+2],y[idxmax-1:idxmax+2])
...
if stickspectrum==1:
          ...
          if np.min(y[j-stickmaxtest:j+stickmaxtest+1]!=0):
            if np.max(y[j-stickmaxtest:j+stickmaxtest+1])==y[j]:
                ...
                if y[j]>t and y[j+1]>t and y[j-1]>t: # peakintensity must be at least t
                    ...
                    valuex, valuey = three_point_interpol(tpi0,tpi1)
    ...
    if stick_dopplerread==True:
        ...
        print('The velocity of the jet is assumed as {:8.1f} m/s'.format(dopplersplitting/2*const.c))
        Doppler_linesx=[]
        Doppler_splits=[]
        Doppler_linesy=[]
        for j in range(len(vx)):
