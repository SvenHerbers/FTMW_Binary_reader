...
label    = "11750_20000shots_"  # Label before index # if JUG data, idx is not used and single file is assumed.
...
masklines=[11750]       # lines to be removed
maskwidth=0.1       # +-width will be removed by mask in MHz.
startidx =  18     # Starting index, you can ommit or add the leading zeros, it works either way. the file indices must consist of 5 digits.
endidx   = 18      # End index, again, the leading zeros are not nessecary but can be added for fun
...
stick_exampledoppler = [[ 12162.97859,         0.03269],[ 12162.97859,         0.03269]] #  - uses two dopplersplittings (+- from centre frequency)  with two frequencies, to calculate the expected dopplersplit throughout the spectrum
